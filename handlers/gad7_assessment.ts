...
Do you understand that this test is a simple way to help you gauge your anxiety for your own reference, and is in no way <b><u>ACTUAL</u></b> mental health services?
    ...
    if (phq9DisclaimerCtx.callbackQuery.data === "gad7-disclaimer-no") {
    ...
    for (const question in gad7Questions) {
      ...
      gad7Ctx = await conversation.waitForCallbackQuery(
        questionCallBackQueries,
      );
...
${gad7Score.action}`,
    ...
    if (userExists(ctx.from.id, dbFile)) {
      ...
      if (settings?.storeMentalHealthInfo) {
        try {
          ...
          logger.error(
            `Failed to save GAD-7 score for user ${ctx.from.id}: ${err}`,
          );
    ...
    logger.error(
      `Error in gad7_assessment conversation for user ${ctx.from?.id}: ${error}`,
    );
    try {
      await ctx.reply(
